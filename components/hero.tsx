...
export function Hero() {
  return (
    <header className="relative isolate overflow-hidden">
      ...
      <div className="relative mx-auto w-full max-w-5xl px-5 pt-[calc(env(safe-area-inset-top,0px)+2.5rem)] pb-12 sm:px-8 sm:pt-[calc(env(safe-area-inset-top,0px)+4rem)] sm:pb-20">
        {/* On a wide screen the copy only fills the left half. The badge takes
            the rest, the way it sits oversized on the door itself. */}
        <div
          aria-hidden
          className="pointer-events-none absolute top-1/2 right-8 hidden w-[26rem] -translate-y-1/2 lg:block"
        >
          <DiamondMark uid="hero-ghost" className="w-full opacity-[0.09]" />
        </div>

        {/* The lockup, as painted on the door */}
        <div className="flex items-center gap-4">
          <DiamondMark uid="hero" lit className="size-[3.25rem] shrink-0 sm:size-20" />
        ...
        <h1 className="text-chrome mt-4 max-w-2xl text-[clamp(1.9rem,7.5vw,3.4rem)] lg:max-w-[33rem] leading-[1.04] font-semibold tracking-[-0.02em]">
